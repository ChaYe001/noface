...
class YawDetecction:
    ...
    def get_pose_estimation(self,img_size, image_points):
        ...
        #print("img Matrix :{}".format(camera_matrix))
    ...
    def get_pose_estimation_in_euler_angle(self,landmark_shape, im_szie):
        try:
            ret, image_points = self.get_image_points_from_landmark_shape(landmark_shape)
            if ret != 0:
                print('get_image_points failed')
                return -1, None, None, None

            ret, rotation_vector, translation_vector, camera_matrix, dist_coeffs = self.get_pose_estimation(im_szie,
                                                                                                       image_points)
            if ret != True:
    ...
    def yaw_detect(self,im):
        ...
        ret, rotation_vector, translation_vector, camera_matrix, dist_coeffs = self.get_pose_estimation(im.shape, image_points)
        if ret != True:
            print('get_pose_estimation failed')
            return ret,0,0,0
