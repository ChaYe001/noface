...
def get_common_cli_parser(parser: argparse.ArgumentParser = None):
    ...
    common_group.add_argument('--data_type',
                              help='All model weights and biases are quantized to FP16.No need to fill in',
                              choices=["FP16"],
                              default='FP16')
...
def get_common_cli_options(model_name):
    ...
    d['data_type'] = ['- Precision of IR', lambda x: 'FP16']
...
def get_onnx_cli_parser(parser: argparse.ArgumentParser = None):
    ...
    return parser
def get_Compile_cli_parser(parser: argparse.ArgumentParser = None):
    """
    Specifies cli arguments for Model Optimizer for ONNX
    ...
    compil_group = parser.add_argument_group('Compile-specific parameters')

    
    # compil_group.add_argument('-h', '--help',action='help', default=SUPPRESS, help='Show this help message and exit.')
    # compil_group.add_argument("-m", "--model",help="Required. Path to an .xml file with a trained model.",
    #                   required=True, type=str)
    compil_group.add_argument("-i", "--inputfile", help="Optional. file from image or camera frame",choices=[1,2],type=int,default=2)
    #args.add_argument("-s","--serviceType", help="Optional. serviceType is common or private,Supported = 0-10",default=1)
    compil_group.add_argument("-t", "--time_slices", help="Optional. time_slices",default=100,type=int)
    compil_group.add_argument("-oo","--output_blob", help="Optional. Path to the output file.default=<model_xml_file>.blob", type=str)
    compil_group.add_argument("-VPU_NUMBER_OF_SHAVES","--VPU_NUMBER_OF_SHAVES", help="Optional. Specifies number of shaves.Should be set with VPU_NUMBER_OF_CMX_SLICES",type=int)
    compil_group.add_argument("-VPU_NUMBER_OF_CMX_SLICES","--VPU_NUMBER_OF_CMX_SLICES", help="Optional. Specifies number of CMX slices.Should be set with VPU_NUMBER_OF_SHAVES",type=int)
...
def get_all_cli_parser():
    ...
    get_Compile_cli_parser(parser=parser)
